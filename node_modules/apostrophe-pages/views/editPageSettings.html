...
{% block modalClass %}
  apos-edit-page-settings apos-ui-modal-no-sidebar apos-ui-modal-has-tabs
{% endblock%}
    ...
    <div class="apos-modal-tabs">
      ...
      <div class="apos-modal-tab apos-active" data-tab-id="groupSettings">
        <div class="apos-modal-tab-content">
          {{ formText('slug', __('Slug')) }}
          {{ formSelect('type', __('Type'), []) }}
          {{ formBoolean('published', __('Published')) }}
          {{ formBoolean('notOrphan', __('Show in Navigation')) }}
          {% block slug %}
          {% endblock %}
          {{ formText('seoDescription', __('SEO Description'), { limit: 155 }) }}
