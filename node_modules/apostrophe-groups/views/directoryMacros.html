  ...
  <div class="apos-person">
    {% if options.full or options.showThumbnail %}
      <div class="apos-thumbnail">
        {{ aposSingleton(person, 'thumbnail', 'slideshow', { size: 'one-sixth' }) }}
      </div>
    {% endif %}
...
{% macro renderGroup(group, options = {}) %}
  <div class="apos-group">
    <div class="apos-thumbnail">
      {{ aposSingleton(group, 'thumbnail', 'slideshow', { size: 'one-sixth' }) }}
    </div>
    <div class="apos-group-heading">
      <h4><a href="{{ group.url }}">{{ group.title | e }}</a></h4>
    </div>
    {% if options.full %}
      <div class="apos-group-body">{{ aposArea(group, 'body') }}</div>
      {% if (group._people) %}
